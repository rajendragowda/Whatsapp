...
import re
import base64
sel_string_prefix= '//*[@title="'
sel_string_postfix = '"]'
chat_grp_string= 'Testing'
chat_grp_name = sel_string_prefix + chat_grp_string + sel_string_postfix
TIMEOUT_SECONDS = 100
...
class WhatsAPIDriver(object):
    ...
    def __init__(self, client="firefox", username="API", proxy=None, command_executor=None, loadstyles=False,
        ...
        if self.client == "firefox":
            ...
            #self.driver = webdriver.Firefox(capabilities=capabilities, options=options, executable_path="C:\\geckodriver.exe")
    ...
    def get_unread(self, include_me=True, include_notifications=False):
        """
...
### Functions added by Rajendra from here on ###

    def get_grp_sender(self):
        try:
            all_senders = self.driver.find_elements_by_xpath("//span[@class='_2a1Yw _1OmDL']")
            latest_msg_sender = all_senders[-1]
        except IndexError:
            latest_msg_sender = NULL
            return (NULL)
        return (latest_msg_sender.text)

    def get_grp_time(self):
        try:
            msgtime = NULL
            all_msg_time = self.driver.find_elements_by_xpath("//span[@class='_3EFt_']")
            msgtime = all_msg_time[-1]
        except IndexError:
            msgtime = NULL
            return (NULL)
        return (msgtime.text)

    def get_grp_msg(self):
        current_msg = self.driver.find_elements_by_xpath(
            "//span[@class='emojitext _2a1Yw emoji-text-clickable']").get_attribute('innerHTML')
        return (current_msg[-1])

    def get_img_link(self):
        current_images = self.driver.find_elements_by_tag_name('img')
        return (current_images[-1].get_attribute('src'))

    def open_grp(self, group_name):
        print('looking for chat group:', group_name)
        chat_grp_name = sel_string_prefix + group_name + sel_string_postfix
        try:
            self.driver.implicitly_wait(TIMEOUT_SECONDS)
            element = self.driver.find_element_by_xpath(chat_grp_name)
            print("found element:", chat_grp_name)
            element.location_once_scrolled_into_view
            self.driver.find_element_by_xpath(chat_grp_name).click()
            print('found chat group:', group_name)
        except TimeoutException:
            raise NoSuchElementException("unable to open %s", group_name)
        else:
            return

    def get_reply(self):
        # reply_chat=self.driver.find_element_by_xpath("//span[@class='quoted-mention']").get_attribute('innerHTML')
        # print(reply_chat)
        # reply_chat=self.driver.find_elements_by_xpath("//span[@class='quoted-mention']")
        # print(reply_chat)

        current_msg = self.driver.find_element_by_xpath(
            "//span[@class='emojitext _2a1Yw emoji-text-clickable']").get_attribute('innerHTML')
        return (current_msg[-1])

    def setup_excel(self, csv_file_name):
        if not os.path.isfile(csv_file_name):
            Excel_tuple = ('Group Name', 'Sender\'s Name', 'Message Content', 'Date', 'Time', '#Tag', 'Media')
            with open(csv_file_name, 'a', newline='') as result:
                writer = csv.writer(result, dialect='excel')
                writer.writerow(Excel_tuple)
                result.close()

    def setup_image(self, images_folder):
        if not os.path.exists(images_folder):
            os.makedirs(images_folder)

    def get_time(self, curr_time):
        return (datetime.fromtimestamp(curr_time).strftime('%H:%M:%S'))

    def get_date(self, curr_time):
        return (datetime.fromtimestamp(curr_time).strftime('%Y-%m-%d'))

    def get_tagcode(self, curr_msg, img_dir, img_filename):
        media_link = 'NA'
        img = re.search("(\/9\w+)", str(curr_msg))
        if img:
            # print("its an image")
            # print(curr_msg)
            media_link = self.write_img_file(curr_msg, img_dir, img_filename)
            curr_msg = "img"
            curr_img = self.get_img_link()
            # print(curr_img)
            tag_code = "#img"

        txt = re.search("(#\w+)", str(curr_msg))
        if txt:
            tag_code = txt.group(1)
        else:
            tag_code = "NA"

        return (tag_code, curr_msg, media_link)

    def write_excel(self, csv_file_name, msg):
        print(msg)
        with open(csv_file_name, 'a', newline='') as result:
            writer = csv.writer(result, dialect='excel')
            writer.writerow(msg)
    ...
    def call_bot(self, curr_msg, contact):
        bot_msg = ""
        curr_msg = str(curr_msg).lower()
        # print (curr_msg)
        bot_msg = bot_dict.bot_resp(curr_msg)
        # print (type(bot_msg))
        if bot_msg != 'Invalid Cmd' and bot_msg != "":
            if str(type(bot_msg)) == '<class \'list\'>' or str(type(bot_msg)) == '<class \'dict_keys\'>' or str(
                    type(bot_msg)) == '<class \'tuple\'>':
                self.send_bot_msg("\n".join(bot_msg), contact)
            else:
                self.send_bot_msg(bot_msg, contact)

    def send_bot_msg(self, send_msg, contact):
        self.send_to_whatsapp_id(contact, send_msg)

    def write_img_file(self, curr_msg, img_dir, img_filename):
        byte_code = str.encode(curr_msg)

        with open(os.path.join(img_dir, img_filename), "wb") as fh:
            fh.write(base64.decodebytes(byte_code))
            fh.close()
        img_link = img_dir + '\\' + img_filename
        img_link_to_hyplink = '=HYPERLINK(\"' + img_link + '\")'
        # print (img_link)
        return (img_link_to_hyplink)
